...
def pixilize_single_image(image, pixel_relative_size):
    ...
    temp = cv2.resize(image, (number_full_pixels_width, number_full_pixels_height), interpolation=cv2.INTER_LINEAR)
...
def pixilize_single_image_with_quantization(image, pixel_relative_size, clusters=8, colorOffset=6):
    ...
    pinformed = np.array([[copy_temp[i], clt.labels_[i], i] for i in range(copy_temp.shape[0])])
    quantzd = temp.copy()

    for i in pinformed:
        quantzd[i[2] // number_full_pixels_width][i[2] % number_full_pixels_width] = centroids[i[1]]

    output = cv2.resize(quantzd, (width, height), interpolation=cv2.INTER_NEAREST)
